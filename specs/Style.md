...
- **Admin divisions:** Each header, status, history note, creation form, configured-divisions list, and timezone form is a direct flow panel. The configured-divisions panel may use `.division-list-panel` in addition to `.panel.flow-panel`; its division rename input is visually limited to `min(40ch, 100%)`.
