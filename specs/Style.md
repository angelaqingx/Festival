...
- **Administration:** Admin child routes use the shared `.application-page-layout > .application-page-content.admin-page-content` structure. Its direct content children are semantic `<section>` or `<details>` elements with `class="panel flow-panel"` (additional component classes are allowed). The panels stack vertically with a `0.85rem` gap, share a visible width of `min(760px, 100%)`, and leave the page background visible between their rounded 18px surfaces. `.flow-panel` is a grid with a `0.85rem` gap. Use `.admin-page-heading` on the heading container for Integrations, Memberships, Divisions, and Settings; its title and supporting text use the same compact grid spacing and zero margins. On hover-capable devices, a direct admin flow panel scales to `1.005` and receives a darker shadow; disable that transition when reduced motion is requested. Prefer `grid` for list rows, settings values, and the two-column membership list/create form within a panel. Use green and gold fills to distinguish accepted/active from pending state. Place the least-frequent workflow, **Settings**, last in the three-column dashboard grid (bottom right at wide sizes).
- **Admin memberships:** Keep the membership list and create-Teacher-Membership form together in one flow panel; render the Accompanist division policy as a following, separate flow panel.
- **Admin divisions:** Each header, status, history note, creation form, and configured-divisions list is a direct flow panel. The configured-divisions panel may use `.division-list-panel` in addition to `.panel.flow-panel`; its division rename input is visually limited to `min(40ch, 100%)`.
- **Admin settings:** Present Organization Timezone and Registration Age Cutoff as separate flow panels. Registration Age Cutoff uses a native date picker and concise text inside the fieldset explaining that registrants’ ages are calculated as of the selected date.
